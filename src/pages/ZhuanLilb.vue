<template>
  <div id="center">
      <div class="title">
            <h4>2013—2018年度<span>{{titleHtml}}</span>专利技术合同构成占比(%)及成交金额（亿元）</h4>
      </div>
      <div id="center-body">
    ...
    methods:{
        zhishicq() {
           ...
           $.get('/ZhiShicq',function(data) {
                ...
                var year = null;
                if(datajson.code === 0) {
                    var result = datajson.data[1];
                    
                    setInterval(function() {
                        if(index >= 7) {
                            index = 1;
                        }
                 year = result.data[0][index];
            ...
            var option = {
                ...
                legend: {
                    ...
                    data: [result.data[1][0],result.data[2][0],result.data[3][0]]
                },
                series : [
                    {
                        name: '访问来源',
